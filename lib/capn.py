...
# %lprun -f chebbasisgen chebbasisgen(stock, npol, a, b, dorder)
...
def chebbasisgen(stock, npol, a, b, dorder = None):
    ...
    bvec = np.column_stack((np.repeat(1, nknots), np.array(z)))

    # Normalize Chebyshev nodes
    for j in np.arange(2, (npol)):
        Tj = np.array((2 * z * bvec[:, j - 1] - bvec[:, j - 2]))
        bvec = np.column_stack((bvec, Tj))

    if dorder is None:
        res = bvec
    
    elif (dorder == 1):
        bvecp = np.column_stack((np.repeat(0, nknots), np.repeat(2/(b - a), nknots)))
        
        # Generate interpolation matrix with n nodes
        for j in np.arange(2, (npol)):
            Tjp = np.array((4/(b - a)) * bvec[:, j - 1] + 2 * z * bvecp[:, j - 1] - bvecp[:, j - 2])  
            bvecp = np.column_stack((bvecp, Tjp))  
...
# %lprun -f vsim vsim(vcoeff, adata, wval)

# resu = vsim(vcoeff, adata, wval)
